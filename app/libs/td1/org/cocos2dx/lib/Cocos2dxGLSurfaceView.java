...
public class Cocos2dxGLSurfaceView extends GLSurfaceView {
    ...
    public static void queueAccelerometer(float f, float f2, float f3, long j) {
        mCocos2dxGLSurfaceView.queueEvent(new Y(f, f2, f3, j));
    }
    ...
    public void onPause() {
        queueEvent(new RunnableC2682aa(this));
        setRenderMode(0);
    }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (action == 0) {
            runnable = new RunnableC2686ca(this, motionEvent.getPointerId(0), fArr[0], fArr2[0]);
        } else if (action != 1) {
            if (action != 2) {
                if (action != 3) {
                    if (action == 5) {
                        ...
                        if (this.mMultipleTouchEnabled || action2 == 0) {
                            runnable3 = new RunnableC2684ba(this, motionEvent.getPointerId(action2), motionEvent.getX(action2), motionEvent.getY(action2));
                        }
                    } else if (action == 6) {
                        int action3 = motionEvent.getAction() >> 8;
                        if (this.mMultipleTouchEnabled || action3 == 0) {
                            runnable3 = new RunnableC2692fa(this, motionEvent.getPointerId(action3), motionEvent.getX(action3), motionEvent.getY(action3));
                        }
                    }
                    ...
                    while (true) {
                        if (i2 >= pointerCount) {
                            ...
                            runnable2 = new Q(this, new int[]{0}, new float[]{fArr[i2]}, new float[]{fArr2[i2]});
                            break;
                        } else {
                            i2++;
                        }
                    ...
                    runnable2 = new S(this, iArr, fArr, fArr2);
                }
                return true;
            } else if (!this.mMultipleTouchEnabled) {
                int i3 = 0;
                while (true) {
                    if (i3 >= pointerCount) {
                        ...
                        runnable2 = new RunnableC2688da(this, new int[]{0}, new float[]{fArr[i3]}, new float[]{fArr2[i3]});
                        break;
                    } else {
                        i3++;
                    }
                ...
                runnable2 = new RunnableC2690ea(this, iArr, fArr, fArr2);
            }
            queueEvent(runnable2);
            return true;
        } else {
            runnable = new P(this, motionEvent.getPointerId(0), fArr[0], fArr2[0]);
        }
        queueEvent(runnable);
        return true;
    }
